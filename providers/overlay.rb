...
action :set do
  ...
  Chef::Log.info "Applying port changes for #{new_resource.name}: to #{ports_conf_path}"
...
action :remove do
  ...
  if ::File.exist?(ports_conf_path)
    Chef::Log.info "Removing port changes for #{new_resource.name}: to #{ports_conf_path}"
